    ...
    private async Task RunAgenticAskAsync(long chatId, string question)
    {
        ...
        else
        {
            var finalPrompt =
                ...
                "Если спрашивают про задачи конкретного человека или зоны (Шпинат=фронтенд, Межайкин=бэкенд) — " +
                "отбери по смыслу ТОЛЬКО относящиеся к нему задачи, остальные не перечисляй. " +
                "Формат пункта: «KAN-NN — краткое описание (статус)». Не выдумывай того, чего нет в данных. " +
